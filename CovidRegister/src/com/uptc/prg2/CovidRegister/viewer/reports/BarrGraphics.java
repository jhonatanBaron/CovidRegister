...
public class BarrGraphics extends JPanel {
	private static final long serialVersionUID = 1L;
	private short x1AxisX, y1AxisX, x2AxisX, y2AxisX,
			x1AxisY, y1AxisY, x2AxisY, y2AxisY;
...
//	}
	...
	public void paintComponent(Graphics graphics) {
		...
		graphics.drawLine(x1AxisX, y1AxisX, x2AxisX,
				y2AxisX);
		graphics.drawLine(x1AxisY, y1AxisY, x2AxisY,
				y2AxisY);
		graphics.setFont(
		...
		this.paintLabelValue(graphics);
		this.paintStatisticalGraphBarrs(graphics2d);
	}
	...
	public double getIntervalX() {
		intervalX = (short) ((x2AxisX - x1AxisX)
				/ tag.size());
		return intervalX;
	}
	...
	private void paintDivisionsX(Graphics graphics) {
		...
		for (short i = (short) x1AxisY; i < getWidth(); i += getIntervalX()) {
			...
			graphics.drawLine(xDivisionX, y1DivisionY,
					xDivisionX, y2DivisionY);
		}
	}
	...
	private void paintDivisionsY(Graphics graphics) {
		...
		for (short i = y2AxisY; i > y1AxisY; i -= getIntervalY()) {
			yDivisionY -= getIntervalY();
			graphics.drawLine(x1DivisionX, yDivisionY,
					x2DivisionX, yDivisionY);
		}
	}
	...
	private void paintGrid(Graphics graphics) {
		...
		for (short i = (short) x1AxisY; i < getWidth(); i += getIntervalX()) {
			xDivisionX += getIntervalX();
			graphics.drawLine(xDivisionX, 0, xDivisionX,
					y1DivisionY);
		}
	...
	public void paintStatisticalGraphBarrs(
			Graphics graphics) {
		graphics.setColor(Color.BLUE);
		for (int i = 0; i < position.size(); i++) {

			double altura = (valueData.get(i) / rankY)
					* getIntervalY();
			double ancho = getIntervalX() / 3;
			double puntoinicialx = getCoordX(
					position.get(i)) - (ancho / 2);
			double puntoinicialY = getCoordY(
					valueData.get(i));
			graphics.fillRect((int) puntoinicialx,
					(int) puntoinicialY, (int) ancho,
					(int) altura);
		}
	}
